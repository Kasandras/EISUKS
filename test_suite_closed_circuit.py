...
class TestSuite:
    ...
    def test_vacancy_search(self):
        ...
        page.click_by_text("Фильтр")
        page.click_by_text("Очистить")
        page.type_source_vacancy("Территориальный орган")
        page.name_source_vacancy(data["organization"])
        page.name_vacant_position(data["staff_unit"])
        page.type_vacancy(data["type_vacancy"])
        page.substitution_competition("Нет")
        page.electronic_documents("Да")
        sleep(1)
        page.profile_activity_organization(data["work_type"])
        page.field_professional_activity(data["job_type"])
        page.key_word(data["key_word"])
        page.click_by_text("Общие сведения")
        page.category_job(data["category_job"])
        # page.group_job(data["group_job]"])
        page.subject_workplace(data["okato_region"])
        sleep(1)
        page.region_workplace(data["okato_area"])
        page.salary_from(str(int(data["salary_from"])*1000))
        page.salary_to(str(int(data["salary_to"])*1000))
        page.business_trip(data["business_trip"])
        page.work_day(data["work_schedule"])
        page.type_service_contract("Неважно")
        page.normal_workday(data["normal_workday"])
        page.click_by_text("Прием документов")
        page.day_start_accept_document_from("22.03.2018")
        page.day_stop_accept_document_to("01.07.2021")
        page.click_by_text("Квалификационные требования")
        page.level_education("Высшее образование – бакалавриат")
        page.service_experience("Без предъявления требований к стажу")
        page.work_experience_speciality("Без предъявления требований к стажу")
        page.click_by_text("Применить")
        page.click_by_text(data["staff_unit"])
